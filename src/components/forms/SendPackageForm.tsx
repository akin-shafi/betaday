...
const PaymentContent = dynamic(
  () => import("@/components/cart/payment-content"),
  {
    ssr: false,
  }
);
...
export function SendPackageForm({ onBack }: SendPackageFormProps) {
  ...
  // Handle "Use my account information" toggle
  useEffect(() => {
  ...
  const protectionFee = formData.packageProtection
    ...
    : 0;
  const baseDeliveryFee = feeData?.deliveryFee || 0;
  ...
  const handlePaymentSuccess = async (paymentData: any) => {
    try {
      ...
      const packageData = {
        userId: user.id,
        pickupAddress: formData.pickupAddress.address,
        pickupCity: formData.pickupAddress.localGovernmentId || "",
        pickupCoordinates: formData.pickupAddress.coordinates || null,
        dropoffAddress: formData.dropoffAddress.address,
        dropoffCity: formData.dropoffAddress.localGovernmentId || "",
        dropoffCoordinates: formData.dropoffAddress.coordinates || null,
        ...
        packageProtection: formData.packageProtection,
        packageWorth: packageWorthNumber,
        ...
        paymentStatus: "paid",
        currency: "NGN",
        distanceInKm: feeData?.distanceInKm || 0,
        pickupZone: feeData?.pickupZone || "",
        dropoffZone: feeData?.dropoffZone || "",
        feeBreakdown: feeData?.breakdown || null,
      };

      console.log("Sending package data:", packageData);

      const response = await fetch(`${baseUrl}/api/package-delivery`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok || !result.data?.packageDelivery) {
        throw new Error(
          result.message ||
            `Failed to create package delivery: ${response.status}`
        );
      }
      ...
      message.success("Package delivery created successfully!");
      router.push(
        `/package-delivery?highlight=${result.data.packageDelivery.id}`
      );
    } catch (error: any) {
      console.error("Error creating package delivery:", error);
      message.error(
        error.message || "Failed to create package delivery. Please try again."
      );
      setActiveTab("form");
    } finally {
        ...
        <div className="flex-1 overflow-y-auto">
          {activeTab === "form" ? (
            <form
              ...
              className="p-4 space-y-4 pb-[80px] sm:pb-0"
            >
              {/* General Error */}
              {errors.general && (
                      ...
                      <div className="flex items-start space-x-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-orange-700 mt-1">
                            This address doesn't provide enough location details
                            for accurate delivery fee calculation. Please select
                  ...
                  {missingLocationError && (
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm text-orange-700">
                        Please select more specific addresses to calculate
                        delivery fee
                      </span>
                    </div>
                  )}
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1 text-left">
                        ...
                        <p className="text-xs text-gray-600">
                          Use our insurance to safeguard your packages against
                          any incidents. We've got you covered!
                        </p>
                      </div>
                    </div>
              ...
              {/* Submit Button */}
              <div className="sticky bottom-0 z-20 bg-white sm:static sm:z-auto">
                <button
                  type="submit"
            ...
            <PaymentContent
              ...
              onBackToCart={() => setActiveTab("form")}
