...
import OpayModal from "@/components/modal/OpayModal";
...
const Cart: React.FC<CartProps> = ({ onClose }) => {
  ...
  const handleOnlinePaymentChoose = (method: string) => {
    ...
    // If Opay is selected, open Opay modal
    if (method === "Pay with Opay") {
  ...
  const handleOpayPaymentSelect = async (opayMethod: string) => {
    try {
      ...
      const paymentData = {
        ...
        channels: [opayMethod.replace("opay_", "")],
      };

      const response = await fetch(`${baseUrl}/api/opay/initialize`, {
        method: "POST",
        headers: {
      ...
      if (result.success && result.data?.authorization_url) {
        ...
        throw new Error(result.message || "Failed to initialize Opay payment");
      }
    } catch (error: any) {
      console.error("Opay payment error:", error);
      message.error(error.message || "Failed to process Opay payment");
      setIsSubmitting(false);
    }
  };

  const handleOpayWalletFund = async (amount: number) => {
    try {
      ...
      if (result.success && result.data?.authorization_url) {
        // Redirect to Opay payment page for wallet funding
        window.location.href = result.data.authorization_url;
      } else {
        throw new Error(
          result.message || "Failed to initialize wallet funding"
        );
      }
    } catch (error: any) {
      console.error("Wallet funding error:", error);
      message.error(error.message || "Failed to fund wallet");
    }
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    // Handle Opay payment separately
    if (displayedPaymentMethod === "Pay with Opay") {
      setIsOpayModalOpen(true);
      return;
    }
  ...
  return (
    <>
      ...
      <OpayModal
        ...
        onSelectMethod={handleOpayPaymentSelect}
        onFundWallet={handleOpayWalletFund}
