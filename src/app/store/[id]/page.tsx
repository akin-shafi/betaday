...
export default function StoreItem() {
  ...
  const getMenuItems = (): MenuItem[] => {
    ...
    const categoryItems =
      activeCategory === "all" ? Object.values(groupedProducts).flat() : groupedProducts[activeCategory] || []
  ...
  const getBusiness = async (businessId: string, selectedProductId?: string | null) => {
    // Prevent duplicate calls
    if (isFetchingRef.current && lastFetchedIdRef.current === businessId) {
      console.log(`🚫 Already fetching business ${businessId}, skipping duplicate call`)
      return
    }
    ...
    try {
      ...
      console.log(`✅ Business fetched: ${businessData.name} (ID: ${businessData.id})`)
      ...
      console.log(`📡 Step 2: Fetching products for business ID: ${businessData.id}`)
      ...
      console.log(`📡 Step 3: Fetching categories for business ID: ${businessData.id}`)
      let apiCategories: string[] = []

      try {
        apiCategories = await fetchCategoriesByBusinessId(businessData.id)
        console.log(`✅ Categories fetched: ${apiCategories.length} items:`, apiCategories)
      } catch (categoryError) {
      ...
      if (products && products.length > 0) {
        ...
        console.log(
          ...
          Object.fromEntries(Object.entries(allGroupedProducts).map(([key, items]) => [key, items.length])),
        )
        ...
        if (apiCategories.length > 0) {
          ...
          finalCategories = apiCategories.filter((category) => allGroupedProducts[category]?.length > 0)
          console.log(`📂 Using API categories (filtered): ${finalCategories.length} items:`, finalCategories)
        }
        ...
        if (selectedProductId && businessWithStatus.status === "open") {
          const selected = products.find((item: Product) => item.id === selectedProductId)
          if (selected) {
      ...
      console.log(`🏁 === FINISHED PROCESSING BUSINESS: ${businessData.name} ===`)
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-white">
              ...
              <>
                ...
                {process.env.NODE_ENV !== "production" && (
                  <div className="mb-4 p-3 bg-gray-100 rounded-lg text-xs text-gray-600 border border-gray-200">
                    <details>
                      <summary className="font-semibold cursor-pointer">Business Hours Debug Info</summary>
                      <div className="mt-2 space-y-1 pl-2">
                        <p>
                          <strong>Business Days:</strong> {business.businessDays}
                        </p>
                        <p>
                        ...
                        <p>
                          <strong>Current Day:</strong> {new Date().toLocaleDateString("en-US", { weekday: "long" })} (
                          {new Date().getDay()})
                        </p>
                        <p>
                        ...
                        <p>
                          <strong>Is Active:</strong> {business.isActive ? "Yes" : "No"}
                        </p>
                      </div>
                    </details>
                  </div>
                ...
                <MenuItemsSection
                  ...
                  businessHours={`${business.businessDays}: ${business.openingTime.substring(0, 5)} - ${business.closingTime.substring(0, 5)}`}
                />
              </>
            ) : (
              <div className="text-center py-12">
                <h3 className="text-lg font-semibold text-gray-900 mb-2">No products available</h3>
                <p className="text-gray-600">This business {"hasn't"} added any products yet.</p>
                <div className="mt-4 p-4 bg-gray-100 rounded-lg text-sm text-gray-600">
                  ...
                  <p>Grouped products keys: {Object.keys(groupedProducts).length}</p>
                  <p>Business has products: {business?.products?.length || 0}</p>
                </div>
              </div>
            )}
          </div>
      ...
      {isOpen && hasProducts && <FloatingCheckoutButton onCheckout={handleCheckout} />}
    </div>
  )
}
