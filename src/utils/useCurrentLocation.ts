...
export function useCurrentLocation(options: UseCurrentLocationOptions = {}) {
  ...
  const googleCheckTimeoutRef = useRef<NodeJS.Timeout | null>(null)
  ...
  useEffect(() => {
    const checkGoogleMapsLoaded = () => {
      if (window.google && window.google.maps && window.google.maps.Geocoder) {
        ...
        if (googleCheckTimeoutRef.current) {
          clearTimeout(googleCheckTimeoutRef.current)
        }
      } else {
        googleCheckTimeoutRef.current = setTimeout(checkGoogleMapsLoaded, 100)
      }
    }
  ...
  const extractLocationDetails = useCallback((addressComponents: google.maps.GeocoderAddressComponent[]) => {
    let state = ""
    let localGovernment = ""
    let locality = ""
    let subLocality = ""
  ...
  const createFormattedAddress = useCallback((details: any) => {
          ...
          geocoderRef.current!.geocode(
            ...
            (results: any[], status: any) => {
              if (status === google.maps.GeocoderStatus.OK && results && results.length > 0) {
                ...
                const bestResult =
                  results.find((result: { types: string[] }) =>
  ...
  }, [isLoading, extractLocationDetails, createFormattedAddress])

  // Set up event listener for manual location fetch
  useEffect(() => {
  ...
  }, [googleLoaded, fetchCurrentLocation])
